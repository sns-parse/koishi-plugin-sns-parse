/**
 * 解析编排（发送层无关）：三层去重 → 并发解析 → gate（NSFW 策略）预处理 → 发送分派。
 *
 * 发送由 OutboundSender 完成（Koishi 适配器见 src/sender/koishi-sender.ts）。
 *
 * 去重三层（默认全开）：
 * ① 同消息 URL 去重（含并发竞态）
 * ② 内容指纹去重（不同链接相同内容只发首个）
...
async function processItem(rt: ParserRuntime, session: any, platform: string, text: string, parsed: any): Promise<ProcessedItem> {
  // runtime 已用默认实现补齐，这里的断言是安全的
  const { mergeImages, processImage, processVideo, processMergedImage, mp4ToGif } = rt.extensions as Required<typeof rt.extensions>
...
export async function flush(rt: ParserRuntime, session: SessionLike, matches: LinkMatch[], opts: FlushOptions = {}) {
  const sender = opts.sender ?? rt.host.sender
  if (!sender) throw new Error('flush: 未提供 OutboundSender（请通过 opts.sender 或 host.sender 注入）')
    ...
    try {
      ...
      if (dedupEnabled) {
        ...
        if (lastTime && (Date.now() - lastTime < config.deduplicationInterval * 1000)) {
          ...
          await sender.send(session, [el.text(tip)]).catch(() => {})
          return
        }
      }
  ...
  if (errors.length) await sender.send(session, [el.text(`${getText(config, 'parseErrorPrefix')}\n${errors.join('\n')}`)])
  if (!items.length) return

  const forwardAllowed = config.enableForward && (session.platform === 'onebot' || session.platform === 'satori')

  if (forwardAllowed) {
    await sendForward(rt, sender, session, items)
    return
  }

  if (config.sendStrategy === 'split') {
    for (const item of items) await sendSplit(rt, sender, session, item, { quoteId: session?.messageId })
    return
  }

  // single（默认）：单条整合；混淆图/视频取件码作为例外独立发送
  for (const item of items) {
    await sendSingle(rt, sender, session, item, { quoteId: session?.messageId })
  }
}
